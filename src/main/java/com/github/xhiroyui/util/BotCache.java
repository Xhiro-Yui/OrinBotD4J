...
public class BotCache {
	...
	// Method call to populate/refresh the mutedUsersCache. Called once upon booting.
	public static int refreshMutedUsersCache() {
		mutedUsersCache = new ArrayList<MutedUser>(); 
		ArrayList<String[]> temp = DBConnection.getDBConnection().selectQueryMultipleColumnMultipleResults("SELECT * FROM " + BotConstant.DB_MUTED_USERS_TABLE);
		if (temp != null) {
			for (String[] each : temp) {
				MutedUser user = new MutedUser();
				user.setUserID(Long.parseLong(each[1]));
				user.setGuildID(Long.parseLong(each[2]));
				user.setTimeStamp(each[3]);
